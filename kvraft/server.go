...
type Op struct {
	...
	Key   string // string
	Value string // if Put or Append, value is a string
...
type KVServer struct {
	...
	// Your definitions here.
	store       map[string]string // to store the string values
	results     map[int]chan Op   // to work on queries sequentially
	lastApplied map[int64]int64   // to check the last applied index associated with each client/clerk
}
...
func (kv *KVServer) waitForApplied(op Op) (bool, Op) {
	index, _, isLeader := kv.rf.Start(op) // sends a command to the log of the leader in raft package
...
func (kv *KVServer) applyOpsLoop() {
	for {
		...
		op := msg.Command.(Op) // type conversion
		...
		if op.Type == "Get" {
			...
			if !ok || op.RequestId > lastId { // if we are facing a new request!
				kv.applyToStateMachine(&op)
				kv.lastApplied[op.ClientId] = op.RequestId // updating the request id of that client/clerk
			}
		}
...
//
func StartKVServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister) *KVServer {
	...
	// initializing of all components
	...
	// You may need initialization code here.
	// go routine to be run concurrently
	go kv.applyOpsLoop()
